...
We found that initial uploads for Medtronic 630G, 640G, and 670G were a bit longer than we would like, especially for our folks uploading in a clinic. So we added a 4-week filter to for Medtronic 630G, 640G, and 670G uploads. This should move things along nicely.  
...
Here's the thing, sometimes you don't notice those requests to install drivers from a new piece of software. It happens. The problem is, if those drivers for Tidepool Uploader aren't installed some of your devices won't upload. That's no fun. Now Tidepool Uploader on macOS will fall back to a user-space driver we wrote that will support Animas pumps, and Ultra2, Ultra Mini, or VerioIQ meters if we detect the original drivers were not installed. Pretty cool, huh?  
