...
*  `schemaVersion` - A "schemaVersion" for the type. The original type will have a `schemaVersion` of `0`, the next modification to the schema for a given type will mean the version will be `1`, and so on.
