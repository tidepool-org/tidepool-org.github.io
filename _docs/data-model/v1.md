...
Repeat: all numerical values for blood glucose data on the Tidepool platform are in `mmol/L`. However, the `units` field describes the units of the value(s) originally displayed to the user. This information is preserved to serve as an indication of whether a client-side application should convert the values to a different set of units (i.e., back to `mg/dL`) before displaying them to a user. This is why it might have been a better idea to name the field `displayUnits`.
